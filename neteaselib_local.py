...
def get_music_info(music_id: str):
    searchResult = requests.get('http://localhost:3000/search?keywords=' + music_id).json()
    musicId = str(searchResult['result']['songs'][0]['id'])
    musicResult = requests.get('http://localhost:3000/song/url?id=' + musicId).json()
    musicUrl = musicResult['data'][0]['url']
    musicTitle = searchResult['result']['songs'][0]['name']
    musicPic = searchResult['result']['songs'][0]['artists'][0]['img1v1Url']
    musicArResult = searchResult['result']['songs'][0]['artists'][0]['name']
